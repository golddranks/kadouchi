...
#[derive(Clone, Debug)]
pub struct Object {
    init: extern "C" fn(u16, *const *const InnerObject) -> InnerObject,
    path: AbsPath2,
...
extern "C" fn emit_root_object(
    arg_count: u16,
    iref_array: *const *const InnerObject,
) -> InnerObject {
    unreachable!("The root object should be unnameable and thus unreferenceable.");
}

extern "C" fn emit_intrinsic_object(
    arg_count: u16,
    iref_array: *const *const InnerObject,
) -> InnerObject {
...
impl Object {
    fn new(init: extern "C" fn(u16, *const *const InnerObject) -> InnerObject) -> Object {
        Object {
...
pub fn check_recursive(
    ...
    for (item_idx, arg) in item.ns.items.iter().enumerate() {
        if let Some(ref referent) = arg.referent {
            ...
            trace!(
                "After initializing its arguments, we are initing the object {:?} itself:\n{:?}",
                arg.local_name,
                arg_object
            );
